...
export async function exportCsv(
  ...
  const request = z
    ...
    .parse(input);
  const catalog = ["customers", "products", "suppliers", "categories"].includes(
    request.kind,
  );
  const load = (page: number) =>
    service.execute(
      catalog ? `${request.kind}.list` : "report.list",
      catalog
        ? { ...request.query, page, page_size: 100 }
        : {
            kind: request.kind,
            query: { ...request.query, page, page_size: 100 },
          },
    ) as Page;
  const result = await dialog.showSaveDialog(window, {
    defaultPath: `${request.kind.replace(/[^a-z_]/g, "")}.csv`,
