...
13. Use Customer fulfilment for appointments, lab orders, repairs/warranty work and scheduled customer follow-ups. Statuses follow controlled transitions and due reminders appear in-app and as native notifications.
14. Use Quotes & reservations to create an expiring quote, optionally reserve its stock, take a deposit, and convert it once into a normal invoice. Cancelling releases reserved stock and records a matching deposit refund; cash refunds require an open drawer with sufficient funds.
15. Use Inventory control to run a physical stocktake, post supplier returns against received purchases, print scannable Code 39 labels and transfer stock between branches.
16. Choose the active branch in the top bar before operating. Sales, purchasing, cash drawers, fulfilment, stock and reports are attributed to that branch. Product master data and customer history remain shared across branches.
...
The local application writes immutable synchronization-outbox events for operational mutations. Actual cloud synchronization is not enabled until a deployment supplies a server URL, authentication/tenant model and conflict policy.
