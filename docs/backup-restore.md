...
Settings now supports native folder selection, automatic scheduling every 1–30 days, optional backup on close and retention of 2–100 automatic backups. Manual backups are not rotated. Scheduling runs while the app is open, not as a Windows background service. Automatic failures are logged; check history and logs regularly. The configured destination overrides the default application data backup folder.
