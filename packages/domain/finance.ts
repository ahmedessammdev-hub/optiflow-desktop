...
export function saleBalance(store: Store, saleId: string) {
  ...
  const paid = Number(
    store.get(
      "SELECT COALESCE(sum(amount),0) n FROM payments WHERE sale_id=?",
      saleId,
    )?.n,
  );
  const returned = Number(
    store.get(
      "SELECT COALESCE(sum(total),0) n FROM returns WHERE sale_id=?",
      saleId,
    )?.n,
  );
  const refunded = Number(
    store.get(
      "SELECT COALESCE(sum(amount),0) n FROM refunds WHERE sale_id=?",
      saleId,
    )?.n,
  );
  return {
    ...
    returned,
    refunded,
    balance: Number(sale.total) - returned - paid + refunded,
  };
}
export function cashEntry(
  ...
  store.insert("cash_transactions", {
    id: randomUUID(),
...
export class Finance {
  ...
  checkout(input: unknown) {
    ...
    return this.store.tx(() => {
      ...
      const customer = data.customer_id
        ? this.store.get(
            "SELECT * FROM customers WHERE id=? AND archived_at IS NULL",
            data.customer_id,
          )
        : null;
      if (data.customer_id && !customer)
        throw new Error("Customer not found or archived");
      const prescription = data.prescription_id
        ? this.store.get(
            "SELECT * FROM prescriptions WHERE id=? AND customer_id=?",
            data.prescription_id,
            data.customer_id,
          )
        : null;
      if (data.prescription_id && !prescription)
      ...
      const amounts = totals(products, data.discount, data.tax);
      const paid = checked(data.payments.reduce((s, p) => s + p.amount, 0));
      if (paid > amounts.total)
        throw new Error("Payment exceeds invoice total");
      if (paid < amounts.total && !customer)
        throw new Error("A customer is required for partial payment");
      const saleId = randomUUID();
      const now = new Date().toISOString();
      const next =
        Number(this.store.get("SELECT count(*) n FROM sales")?.n) + 1;
      this.store.insert("sales", {
        id: saleId,
        request_id: data.request_id,
        invoice_number: `INV-${String(next).padStart(7, "0")}`,
        ...
        prescription_snapshot: prescription
          ? JSON.stringify({
              ...JSON.parse(String(prescription.data)),
              id: prescription.id,
      ...
      const taxes = allocate(data.tax, weights);
      products.forEach(({ p, item, unit_price, quantity }, i) => {
        this.store.insert("sale_items", {
          ...
          net_total: unit_price * quantity - discounts[i] + taxes[i],
          tax_amount: taxes[i],
      ...
      for (const payment of data.payments)
        this.addPayment(saleId, payment, user.id, "sale_payment");
      this.store.audit(user.id, "checkout", "sales", saleId, null, {
        ...amounts,
        paid,
      });
      return saleId;
  ...
  payment(input: unknown) {
    ...
    const data = paymentSchema.extend({ sale_id: id }).parse(input);
    this.store.tx(() => {
      if (data.amount > saleBalance(this.store, data.sale_id).balance)
      ...
      this.store.audit(user.id, "payment", "sales", data.sale_id, null, data);
    });
  }
  reverse(input: unknown) {
    const user = this.auth.require("payments.reverse");
    const data = z.object({ payment_id: id, reason: text.min(1) }).parse(input);
    this.store.tx(() => {
      ...
      if (
        ...
        this.store.get(
          "SELECT id FROM payments WHERE reversal_of=?",
          data.payment_id,
        )
      )
  ...
  returnItems(input: unknown) {
    ...
    const data = z
      ...
      .parse(input);
    if (
      new Set(data.items.map((i) => i.sale_item_id)).size !== data.items.length
    )
    ...
    return this.store.tx(() => {
      ...
      const lines = data.items.map((item) => {
        ...
        const returned = this.store.get(
          "SELECT COALESCE(sum(quantity),0) quantity,COALESCE(sum(amount),0) amount,COALESCE(sum(tax_amount),0) tax_amount FROM return_items WHERE sale_item_id=?",
          item.sale_item_id,
        )!;
        if (
          item.quantity + Number(returned.quantity) >
          Number(original.quantity)
        )
          throw new Error("Return exceeds remaining sold quantity");
        const cumulative = Number(
          (BigInt(Number(original.net_total)) *
            BigInt(item.quantity + Number(returned.quantity))) /
            BigInt(Number(original.quantity)),
        );
        const amount = cumulative - Number(returned.amount);
        const taxAmount =
          Number(
            (BigInt(Number(original.tax_amount)) *
              BigInt(item.quantity + Number(returned.quantity))) /
              BigInt(Number(original.quantity)),
          ) - Number(returned.tax_amount);
        total += amount;
        return { item, original, amount, taxAmount };
      });
      this.store.insert("returns", {
        id: returnId,
      ...
      for (const { item, original, amount, taxAmount } of lines) {
        this.store.insert("return_items", {
          id: randomUUID(),
          return_id: returnId,
          ...
          restock: Number(item.restock),
        });
        if (item.restock)
          moveStock(
            this.store,
            String(original.product_id),
            item.quantity,
            "sale_return",
            returnId,
            data.reason,
      ...
      this.store.audit(user.id, "return", "sales", data.sale_id, balance, {
        return_id: returnId,
        total,
        refund,
      });
  ...
  invoice(input: unknown) {
    ...
    const saleId = id.parse(input);
    const sale = this.store.get(
      "SELECT s.*,u.name seller_name FROM sales s JOIN users u ON u.id=s.seller_id WHERE s.id=?",
      saleId,
    );
    if (!sale) throw new Error("Invoice not found");
    const items = this.store.all(
      "SELECT si.*,COALESCE((SELECT sum(quantity) FROM return_items WHERE sale_item_id=si.id),0) returned_quantity FROM sale_items si WHERE sale_id=?",
      saleId,
    );
    if (!this.auth.require().permissions.includes("reports.profit"))
      items.forEach((i) => delete i.unit_cost);
    const canPrescription = this.auth
      .require()
    ...
    return {
      ...
      returns: this.store.all(
        "SELECT * FROM returns WHERE sale_id=? ORDER BY created_at",
        saleId,
      ),
      ...saleBalance(this.store, saleId),
    };
  }
}
